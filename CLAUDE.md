...
- **Masa Yoldaşı (Desk Companion: Cat, Dog, Rabbit, Fox)** — an inline SVG (`renderCozyDecorations()`, `app.js`) with a CSS breathing-belly loop and custom animations for each pet (cat tail/ears, puppy floppy ears/wagging tail, rabbit long twitchy ears/pom tail, fox fluffy tail/ears), drawn with a knit nightcap automatically between 22:00–06:00 local time (`new Date().getHours()` check, re-evaluated on every render — not persisted). Supports **Awake / Study Buddy** mode (`isPetAwake()`, `wakeUpPet()`, eyes open, natural CSS blink `@keyframes petEyeBlink`, reading glasses during research phase). Clicking/tapping it (`#cozyCatBtn` handler in `bindStageEvents()`) wakes it up, pets it, and routes to animal-specific sound synthesizers/samples (`playCatInteraction`, `playDogSound`, `playRabbitSound`, `playFoxSound`). 3+ rapid clicks flip it to `"annoyed"`. Persisted as `settings.petType`, `settings.petColor`, and `settings.petBehavior` (`"smart"` | `"awake"` | `"sleep"`) in `settings`.
